...
div1.innerHTML = `
  <div class="bg-white max-w-2xl w-full p-8 text-center rounded-lg shadow-lg shadow-red-500">
    ...
    <button type="button" class="click bg-red-500 p-4 text-white rounded shadow-lg text-2xl font-medium">
      Let's Dive
    </button>
  </div>
`;
...
document.querySelector(".click").addEventListener("click", function(e) {
  ...
  form2.innerHTML = `
    <div class="bg-white max-w-2xl w-full p-9 text-center rounded-lg shadow-lg shadow-red-500 min-h-[350px]">
      ...
      <button type="button" id="yesBtn" class="bg-green-500 text-white px-6 py-2 mt-3 rounded-lg shadow-lg mr-4">Yes 💖</button>
      <button type="button" id="noBtn" class="bg-gray-500 text-white px-6 py-2 mt-3 rounded-lg shadow-lg">No 💔</button>
...
// No button
let noBtn = form2.querySelector("#noBtn");
let resultDiv = form2.querySelector("#result");
let card = form2.querySelector("div.bg-white"); // the white card container
...
function moveButton(button) {
  ...
  // Calculate max positions inside card
  let maxX = cardRect.width - button.offsetWidth;
  let maxY = cardRect.height - button.offsetHeight;

  // Random position anywhere inside card
  ...
  // Ensure button stays inside card boundaries
  button.style.margin = "0";
  card.style.position = "relative"; // important: card must be relative
}

noBtn.addEventListener("mouseover", function() {
  moveButton(noBtn);
  resultDiv.innerText = messages[msgIndex];
  msgIndex = (msgIndex + 1) % messages.length;
});
